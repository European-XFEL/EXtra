...
class DelayLineDetector:
    ...
    def _get_reduced_pd(self, key, pulse_dim, entry_level, split_axis):
        """Get variable-length data as series or dataframe.

        Load and reduce data with a variable number of actual entries
        among a fixed number of potential entries to build a linear
        pandas Series or DataFrame.

        The data may be 2-dimensional or 3-dimensional with separation
        along one axis. In both cases, the first axis is assumed to be a
        entry ("pulse") dimension and the second to only contain actual
        data up to a variable number for each entry.

        The data type must either be a scalar floating type (returning
        Series objects) or a structured data type (returning DataFrame
        objects) containing at least a single floating type.

        Args:
            key (str): Instrument source key.
            pulse_dim (str): Pulse to use in index, must be supported by
                underlying pulses object.
            entry_level (str or None): Additional index level inserted
                for each entry or omitted if None.
            split_axis (int or None): Axis along which to separate data
                into multiple dataframes, or a single dataframe if None.

        Returns:
            (List of pandas objects or pandas object): List of pandas
                objects with separation enabled or a single pandas
                object. Series objects are returned for scalar data and
                DataFrame objects for structured data.
        """

        kd = self._instrument_src[key]
        pd_cls = pd.DataFrame if kd.dtype.fields is not None else pd.Series

        # Compare counts across data and pulse information to allow all
        # code further below to assume they are aligned.
        if (data_counts := kd.data_counts()).sum() == 0:
            # Return immediately if there are no data counts.
        ...
        if split_axis is None:
            # Separation disabled, insert a virtual axis in the front.
            data = data[None, :]
            axis = 0
        else:
            # Separation enabled.
        ...
        for i in range(data.shape[axis]):
            # Part of data to be handled in this iteration.
            subdata = np.take(data, i, axis=axis)

            num_rows = subdata.shape[1]
            raw = subdata.ravel()
            ...
            res.append(pd_cls(
                raw[finite_mask], pd.MultiIndex.from_frame(index_df)))

        return res if split_axis is not None else res[0]
    ...
    def edges(self, channel_index=True, pulse_dim='pulseId'):
        ...
        raw_edges = self._get_reduced_pd('raw.edges', pulse_dim, None, 1)
    ...
    def hits(self, pulse_dim='pulseId'):
        ...
        return self._get_reduced_pd(
            'rec.hits', pulse_dim, 'hitIndex', None)
