...
def test_train_data(mock_sqs_remi_run):
    ...
    for data in [xr_data.data, np_data]:
        ...
        assert np.all(data.argmax(axis=1) == 8974)
...
def test_train_edge_array(mock_sqs_remi_run):
    ...
    for edges, amplitudes in [
        ...
        assert np.all(edges[finite_mask] == 8944.0)
        assert np.all(amplitudes[finite_mask] == 5.0)
...
def test_train_edges(mock_sqs_remi_run):
    ...
    assert np.all(df.columns == ['edge', 'amplitude'])
    assert np.all(df.index.names == ['trainId', 'edgeIndex'])
    assert len(df) == 100

    assert np.isclose(df['edge'], 8944.0).all()
    assert np.isclose(df['amplitude'], 5.0).all()
...
def test_pulse_edge_array(mock_sqs_remi_run):
    ...
    assert np.all(ds.edges.data[finite_mask] == 888.0)
    assert np.all(ds.amplitudes.data[finite_mask] == 5.0)
...
def test_pulse_edges(mock_sqs_remi_run):
    ...
    assert isinstance(df, pd.DataFrame)
    assert np.all(df.columns == ['edge', 'amplitude'])
    assert np.all(df.index.names == ['trainId', 'pulseId', 'edgeIndex'])
    assert len(df) == 40

    assert np.all(df.index.get_level_values('trainId') == np.r_[10010:10050])
    assert np.all(df.index.get_level_values('pulseId') == 1018)
    assert np.all(df.groupby(['trainId', 'pulseId']).count() == 1)
    assert np.all(df['edge'] == 888.0)
    assert np.all(df['amplitude'] == 5.0)
