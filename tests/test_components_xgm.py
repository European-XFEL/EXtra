...
def test_multisase_xgm(multi_xgm_run):
    ...
    assert xgm.intensity().name == f"{instrument}.data.intensitySa3TD"
    ...
    assert xgm.intensity().name == f"{instrument}.data.intensitySa1TD"
    assert xgm.pulse_counts().name == f"{control}.pulseEnergy.numberOfSa1BunchesActual"

    # Test specifying an explicit non-default SASE
    assert xgm.intensity(0).name == f"{instrument}.data.intensityTD"
    assert xgm.pulse_counts(0).name == f"{control}.pulseEnergy.numberOfBunchesActual"

    assert xgm.intensity(3).name == f"{instrument}.data.intensitySa3TD"
    assert xgm.pulse_counts(3).name == f"{control}.pulseEnergy.numberOfSa3BunchesActual"

    # Smoke tests
    mock_intensity = xr.DataArray(np.random.rand(1000, 100),
                                  dims=("trainId", "pulseIndex"))
    with patch.object(xgm, "intensity", return_value=mock_intensity):
        # Test both a specific SASE (the current `default_sase`) and the main
        # properties (with `sase=0`).
        xgm.plot_intensity()
        xgm.plot_intensity(0)
        xgm.plot_pulse_intensity()
        xgm.plot_pulse_intensity(0)
        xgm.plot_train_intensity()
        xgm.plot_train_intensity(0)
        xgm.plot()
